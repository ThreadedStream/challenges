...
enum {
	...
	G_IDLE = 0,
		// G_RUNNABLE means this goroutine is on a run queue. It is
	...
	G_SCAN          = 0x1000,
	G_SCAN_NRUNNABLE  = G_SCAN + G_RUNNABLE,  // 0x1001
	G_SCAN_RUNNING   = G_SCAN + G_RUNNING,   // 0x1002
	G_SCAN_SYSCALL   = G_SCAN + G_SYSCALL,   // 0x1003
	G_SCAN_WAITING   = G_SCAN + G_WAITING,   // 0x1004
	G_SCAN_PREEMPTED = G_SCAN + G_PREEMPTED, // 0x1009
};

enum {
	WAIT_REASON_ZERO = 0, // ""
	WAIT_REASON_GC_ASSIST_MARKING,                         // "GC assist marking"
	WAIT_REASON_IO_WAIT,                                  // "IO wait"
	WAIT_REASON_CHAN_RECEIVE_NIL_CHAN,                      // "chan receive (nil chan)"
	WAIT_REASON_CHAN_SEND_NIL_CHAN,                       // "chan send (nil chan)"
	WAIT_REASON_DUMPING_HEAP,                             // "dumping heap"
	WAIT_REASON_GARBAGE_COLLECTION,                       // "garbage collection"
	WAIT_REASON_GARBAGE_COLLECTION_SCAN,                   // "garbage collection scan"
	WAIT_REASON_PANIC_WAIT,                               // "panicwait"
	WAIT_REASON_SELECT,                                  // "select"
	WAIT_REASON_SELECT_NO_CASES,                           // "select (no cases)"
	WAIT_REASON_GC_ASSIST_WAIT,                            // "GC assist wait"
	WAIT_REASON_GC_SWEEP_WAIT,                             // "GC sweep wait"
	WAIT_REASON_GC_SCAVENGE_WAIT,                          // "GC scavenge wait"
	WAIT_REASON_CHAN_RECEIVE,                             // "chan receive"
	WAIT_REASON_CHAN_SEND,                                // "chan send"
	WAIT_REASON_FINALIZER_WAIT,                           // "finalizer wait"
	WAIT_REASON_FORCE_GC_IDLE,                             // "force gc (idle)"
	WAIT_REASON_SEMACQUIRE,                              // "semacquire"
	WAIT_REASON_SLEEP,                                   // "sleep"
	WAIT_REASON_SYNC_COND_WAIT,                            // "sync.Cond.Wait"
	WAIT_REASON_TIMER_GOROUTINE_IDLE,                      // "timer goroutine (idle)"
	WAIT_REASON_TRACE_READER_BLOCKED,                      // "trace reader (blocked)"
	WAIT_REASON_WAIT_FOR_GC_CYCLE,                          // "wait for GC cycle"
	WAIT_REASON_GC_WORKER_IDLE,                            // "GC worker (idle)"
	WAIT_REASON_PREEMPTED,                             // "preempted"
	WAIT_REASON_DEBUG_CALL,                               // "debug call"
};
...
typedef struct forcegcstate forcegcstate_t;
...
struct ancestor_info {
	uintptr_t *pcs;  // pcs from the stack of this goroutine
	int64_t goid;      // goroutine id of this goroutine; original goroutine possibly dead
